 ...
 * div in html, the divs are postioned using absolute position system
 */
...
for(file <- args) { 
    ...
    val newFileName = filePathSplit.last.split("[.]").head + "Absolute.html"
    ...
    val rows: scala.xml.NodeSeq = 0 until h map { y => 
        ...
        val pixels: scala.xml.NodeSeq = 0 until w map { x => 
            ...
            val b = pixel.getBlue
            <div class="pixel" style={s"background-color:rgb($r, $g, $b); top: ${y}px; left: ${x}px;"}></div> 
        } toSeq 
